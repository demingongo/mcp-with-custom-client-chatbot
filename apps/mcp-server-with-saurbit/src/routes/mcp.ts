...
export const mcpPostRoute = applyModifiers(
  withSchema({
    headers: z
      .looseObject({
      ...
      if (Boom.isBoom(err)) {
        ...
        if (issue) {
          if (issue.path[0] === "payload") {
            ...
            if (issue.path.length === 1 && issue.message.endsWith("received array")) {
            ...
            if (issue.path[1] === "mcp-protocol-version") {
    ...
    options: {
      auth: {
        strategy: multipleFlowsPlugin.getStrategyName(),
      },
  ...
  }),
  {
    responses: groupResponses(
      new ResponseDocsModifier().setDescription("No content (for notifications)").setCode(202),
      ...
      new ResponseDocsModifier().addMediaType("text/event-stream").setName("MCP SSE Stream")
    ),
  }
);
...
export const mcpDeleteRoute = applyModifiers(
  ...
  {
    responses: groupResponses(
      new ResponseDocsModifier().setDescription("Session ended successfully").setCode(204),
      new ResponseDocsModifier()
...
export const mcpGetRoute = applyModifiers(
  withSchema({
    headers: z
      .looseObject({
  ...
  }),
  {
    responses: groupResponses(
      new ResponseDocsModifier()
        ...
        .addMediaType("text/event-stream")
