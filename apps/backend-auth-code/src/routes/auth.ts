...
export const loginRoute: KaapiServerRoute = {
  ...
  options: {
    ...
    notes:
      "Returns authorizationUrl to redirect the user's browser to. Returns alreadyAuthenticated: true if the user already has valid tokens. Returns userId when a new session was created.",
    tags: ["auth"],
    plugins: {
      kaapi: {
        docs: false,
...
export const callbackRoute: KaapiServerRoute = {
  ...
  options: {
    ...
    notes:
      "Receives the authorization code from the OAuth server and exchanges it for tokens. Redirects the browser to the frontend on success.",
    tags: ["auth"],
    plugins: {
      kaapi: {
        docs: false,
